...
impl ExtensionField for fp_256::Monty {
    ...
    fn frobenius_factor_1() -> Fp2Elem<Self> {
        Fp2Elem {
            ...
            elem1: fp_256::Monty::new([12717650, 1979656862, 1972108829, 1559171847, 290869350, 694266310, 579853055, 1597302116, 4]),
            // 15931493369629630809226283458085260090334794394361662678240713231519278691715
            elem2: fp_256::Monty::new([916222744, 723717387, 574787027, 1552331516, 1261698828, 887614939, 1510504974, 237248389, 8]),
        }
    }
    #[inline]
    fn frobenius_factor_2() -> Fp2Elem<Self> {
        Fp2Elem {
            ...
            elem1: fp_256::Monty::new([15189756, 1959629061, 767155255, 1427131279, 513871136, 1996531885, 1401068065, 1422686260, 66]),
            // 21645619881471562101905880913352894726728173167203616652430647841922248593627
            elem2: fp_256::Monty::new([656635847, 1575335407, 606381995, 1750621660, 1486126693, 503904416, 1955368925, 270969940, 28]),
        }
    }
    #[inline]
    fn frobenius_factor_fp12() -> Fp2Elem<Self> {
        Fp2Elem {
            ...
            elem1: fp_256::Monty::new([454717458, 161507933, 804946138, 817080316, 1413496639, 904558052, 1356825578, 765259087, 11]),
            // 19998038925833620163537568958541907098007303196759855091367510456613536016040
            elem2: fp_256::Monty::new([392395988, 386079276, 1435458002, 829532913, 1153717094, 1170750470, 311764801, 1868664732, 53]),
        }
    }
    ...
    fn v() -> Fp6Elem<Self> {
        Fp6Elem {
            ...
            elem2: Fp2Elem {
                elem1: Zero::zero(),
                elem2: fp_256::Monty::new([1368961080,1174866893, 1632604085, 2004383869, 1511972380, 1964912876, 1176826515, 403865604, 70]),
            },
            elem3: Zero::zero(),
        }
    }
...
impl ExtensionField for fp_480::Monty {
    fn xi() -> Fp2Elem<Self> {
        Fp2Elem {
            elem1: fp_480::Monty::new([1588384315, 657481659, 1879608514, 2019977405, 241404753, 1339062904, 639566708, 740072562, 1004131918, 1560224833, 2014075, 1848411426, 1733309265, 1811487384, 799788540, 19667]),
            elem2: fp_480::Monty::new( [1562186266, 585547362, 445363961, 373107586, 235669429, 70320378, 1725584133, 1655137918, 674554062, 1273113365, 570248622, 1027777883, 144574781, 808941279, 1969907309, 26235]),
        }
    }
    // precalculate this since it's used in every double and add operation in the extension field.
    // Fp480::xi().inv() * 9
    fn xi_inv_times_9() -> Fp2Elem<Self> {
        Fp2Elem {
            elem1: fp_480::Monty::new([1671524251, 1452836661, 1514278714, 1089055752, 983710563, 1342902855, 1277558075, 1876974179, 947518822, 981090444, 247487949, 1283900828, 1334523677, 1392777601, 1513546415, 5235]),
            elem2: fp_480::Monty::new([335877574, 1323354927, 650625437, 272173725, 1832380438, 2065643415, 655409061, 1376608173, 1642768871, 464289801, 840813404, 665753910, 1051781983, 447188070, 183786362, 17059]),
        }
    }
    fn frobenius_factor_1() -> Fp2Elem<Self> {
        Fp2Elem {
            ...
            elem1: fp_480::Monty::new([1789717966, 1667803516, 415596737, 273473839, 86323342, 1847146047, 411641533, 883838890, 333532818, 2020025246, 1090726896, 1144837448, 1056047014, 650675300, 1518220857, 28583] ),
            ...
            elem2: fp_480::Monty::new([1463808839, 1670428701, 447968177, 1822223094, 1576270537, 1038296259, 66655687, 578662642, 153246326, 1500216407, 53886423, 1315635309, 1456804263, 176996090, 1710325846, 31113]),
        }
    }
    ...
    fn frobenius_factor_2() -> Fp2Elem<Self> {
        Fp2Elem {
            ...
            elem1: fp_480::Monty::new([721025432, 530063334, 1045885399, 453726440, 2141696003, 1626330876, 1074975251, 1714081964, 1995410861, 609055642, 2058665923, 1876157870, 937690781, 1255299233, 741827240, 14111]),
            ...
            elem2: fp_480::Monty::new([147532118, 4451373, 893741577, 424324553, 481155820, 353715416, 620523272, 346497951, 1670022807, 1427377181, 732861676, 2088692375, 582789470, 669478976, 249410141, 28740]),
        }
    }
    ...
    fn frobenius_factor_fp12() -> Fp2Elem<Self> {
        Fp2Elem {
            ...
            elem1: fp_480::Monty::new([736041138, 285900164, 1852287620, 1099722104, 1782561133, 1457388007, 1874229037, 1965266811, 1231149838, 771491782, 467524787, 1737481601, 782773382, 79418783, 1572898018, 31139]),
            ...
            elem2: fp_480::Monty::new([492638839, 304045155, 1057698408, 772372983, 1813080623, 1701111681, 801357526, 1606643950, 1537923020, 1695901916, 157173875, 1177883133,354458948, 163418612, 376825454, 23056]),
        }
    }
    ...
    fn v() -> Fp6Elem<Self> {
        Fp6Elem {
            ...
            elem2: Fp2Elem {
                ...
                elem2: fp_480::Monty::new([1588384315, 657481659, 1879608514, 2019977405, 241404753, 1339062904, 639566708, 740072562, 1004131918, 1560224833, 2014075, 1848411426, 1733309265, 1811487384, 799788540, 19667]),
            },
            elem3: Zero::zero(),
        }
    }
